...
#define UART_BUFFER_SIZE 1024 // 1 KB per UART send
extern UART_HandleTypeDef huart8; // Or whichever UART you're using




char predicted_class[50];

void Get_NetworkOutput(AppConfig_TypeDef *App_Config_Ptr)
{
    sprintf(predicted_class, "%s %.0f%%",
            App_Config_Ptr->nn_output_labels[App_Config_Ptr->ranking[0]],
            *((float*)(App_Config_Ptr->nn_output_buffer)+0) * 100);
}
void Send_Prediction_UART(AppConfig_TypeDef *App_Config_Ptr)
{
    char msg[70];
    sprintf(msg, "%s\n",
            App_Config_Ptr->nn_output_labels[App_Config_Ptr->ranking[0]]);
...
int main(void)
{
  ...
  while(1)
  {
    ...
    Get_NetworkOutput(&App_Config);
    Send_Prediction_UART(&App_Config);
...
static void MX_UART8_Init(void)
{
    __HAL_RCC_UART8_CLK_ENABLE();  // Enable UART8 clock
...
static void MX_GPIO_Init(void)
{
    ...
    GPIO_InitStruct.Pin = GPIO_PIN_14;  // TX
    ...
    GPIO_InitStruct.Pin = GPIO_PIN_9;  // RX
    GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
    HAL_GPIO_Init(GPIOG, &GPIO_InitStruct);
}
